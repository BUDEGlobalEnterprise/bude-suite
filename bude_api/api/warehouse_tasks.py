"""Public API wrappers for `warehouse_tasks.py`.

Business logic lives in `bude_api.services.inventory.warehouse_tasks`.
...
try:
    ...
    frappe = None
_PUBLIC_NAMES = {'list_open', 'complete'}
from ..services.inventory import warehouse_tasks as _service
...
def list_open(limit: int=100, company: str | None=None):
    return _call("list_open", limit, company)

@_whitelist(methods=["POST"])
def complete(todo_name: str, result_doctype: str | None=None, result_name: str | None=None):
    return _call("complete", todo_name, result_doctype, result_name)
