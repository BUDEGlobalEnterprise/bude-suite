"""Public API wrappers for `warehouses.py`.

Business logic lives in `bude_api.services.inventory.warehouses`.
...
try:
    ...
    frappe = None
_PUBLIC_NAMES = {'get_stock', 'list', 'list_locations'}
from ..services.inventory import warehouses as _service
...
@_whitelist()
def get_stock(warehouse: str, limit: int=100):
    return _call("get_stock", warehouse, limit)

@_whitelist()
def list(limit: int=100, company: str | None=None):
    return _call("list", limit, company)

@_whitelist()
def list_locations(warehouse: str, limit: int=100, company: str | None=None):
    return _call("list_locations", warehouse, limit, company)
