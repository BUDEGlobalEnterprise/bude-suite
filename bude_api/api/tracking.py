"""Public API wrappers for `tracking.py`.

Business logic lives in `bude_api.services.inventory.tracking`.
...
try:
    ...
    frappe = None
_PUBLIC_NAMES = {'batches', 'serials'}
from ..services.inventory import tracking as _service
...
@_whitelist()
def batches(item_code: str, warehouse: str | None=None, include_expired: bool | str=False, limit: int=100):
    return _call("batches", item_code, warehouse, include_expired, limit)

@_whitelist()
def serials(item_code: str, warehouse: str | None=None, batch_no: str | None=None, limit: int=100):
    return _call("serials", item_code, warehouse, batch_no, limit)
