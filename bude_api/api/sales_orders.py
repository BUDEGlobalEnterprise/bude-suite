"""Public API wrappers for `sales_orders.py`.

Business logic lives in `bude_api.services.inventory.sales_orders`.
...
try:
    ...
    frappe = None
_PUBLIC_NAMES = {'list_open', 'get', 'create_delivery_note'}
from ..services.inventory import sales_orders as _service
...
@_whitelist()
def list_open(limit: int=50, offset: int=0, company: str | None=None):
    return _call("list_open", limit, offset, company)

@_whitelist()
def get(name: str):
    return _call("get", name)

@_whitelist()
def create_delivery_note(sales_order: str, source_warehouse: str, items: list, posting_date: str | None=None, company: str | None=None, source_location: str | None=None, pod_attachments: list | None=None):
    return _call("create_delivery_note", sales_order, source_warehouse, items, posting_date, company, source_location, pod_attachments)
