"""Public API wrappers for `alerts.py`.

Business logic lives in `bude_api.services.analytics.alerts`.
...
try:
    ...
    frappe = None
_PUBLIC_NAMES = {'list_alerts', 'low_stock', 'summary'}
from ..services.analytics import alerts as _service
...
@_whitelist()
def list_alerts(limit: int | None=None, offset: int=0):
    return _call("list_alerts", limit, offset)

@_whitelist()
def low_stock(warehouse: str | None=None, limit: int | None=None, offset: int=0):
    return _call("low_stock", warehouse, limit, offset)

@_whitelist()
def summary(warehouse: str | None=None):
    return _call("summary", warehouse)
