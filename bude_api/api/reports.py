"""Public API wrappers for `reports.py`.

Business logic lives in `bude_api.services.analytics.reports`.
...
try:
    ...
    frappe = None
_PUBLIC_NAMES = {'asset_summary', 'asset_register', 'maintenance_history', 'asset_utilization'}
from ..services.analytics import reports as _service
...
@_whitelist()
def asset_summary():
    return _call("asset_summary")

@_whitelist()
def asset_register(location: str | None=None, status: str | None=None, category: str | None=None, limit: int=500):
    return _call("asset_register", location, status, category, limit)

@_whitelist()
def maintenance_history(asset: str | None=None, limit: int=100):
    return _call("maintenance_history", asset, limit)

@_whitelist()
def asset_utilization(days: int=90, limit: int=100):
    return _call("asset_utilization", days, limit)
